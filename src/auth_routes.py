    ...
    if form.validate_on_submit():
        try:
            with get_session() as db_session:
                ...
                existing_email = (
                    db_session.query(User).filter_by(email=form.email.data).first()
                )
                if existing_email:
        ...
        try:
            with get_session() as db_session:
                ...
                if form.email.data != user.email:
                    existing_email = (
                        db_session.query(User)
                        .filter(User.email == form.email.data, User.id != user.id)
                        .first()
                    )
                    if existing_email:
...
def revoke_api_token(token_id: int):
    ...
    try:
        with get_session() as db_session:
            token = (
                db_session.query(SessionToken)
                .filter_by(id=token_id, user_id=current_user.id)
                .first()
            )
...
def api_login():
    ...
    try:
        ...
        with get_session() as db_session:
            ...
            if user.is_locked():
                return jsonify({"success": False, "error": "Account locked"}), 423
...
def api_logout():
    ...
    try:
        ...
        with get_session() as db_session:
            session_token = (
                db_session.query(SessionToken).filter_by(token=token).first()
            )
            if session_token:
...
def api_user_info():
    ...
    try:
        ...
        def _get_user_info():
            ...
            return jsonify(
                {"success": True, "user": user.to_dict(include_sensitive=True)}
            )
