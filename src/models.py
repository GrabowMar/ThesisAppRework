...
class PortConfiguration(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("model", "app_num", name="unique_model_app_port"),
    )
...
class GeneratedApplication(db.Model):
    ...
    generation_status = db.Column(
        db.Enum(AnalysisStatus), default=AnalysisStatus.PENDING
    )
    ...
    # Relationships
    security_analyses = db.relationship(
        "SecurityAnalysis",
        backref="application",
        lazy=True,
        cascade="all, delete-orphan",
    )
    performance_tests = db.relationship(
        "PerformanceTest",
        backref="application",
        lazy=True,
        cascade="all, delete-orphan",
    )
    zap_analyses = db.relationship(
        "ZAPAnalysis", backref="application", lazy=True, cascade="all, delete-orphan"
    )
    openrouter_analyses = db.relationship(
        "OpenRouterAnalysis",
        backref="application",
        lazy=True,
        cascade="all, delete-orphan",
    ...
    def get_directory_path(self) -> str:
        ...
        metadata = self.get_metadata()
        return metadata.get(
            "directory_path", f"misc/models/{self.model_slug}/app{self.app_number}"
        )
...
class SecurityAnalysis(db.Model):
    ...
    application_id = db.Column(
        db.Integer,
        db.ForeignKey("generated_applications.id"),
        nullable=False,
        index=True,
    )
    ...
    status = db.Column(
        db.Enum(AnalysisStatus), default=AnalysisStatus.PENDING, index=True
    )
...
class PerformanceTest(db.Model):
    ...
    test_type = db.Column(
        db.String(50), default="load_test"
    )  # load_test, stress_test, spike_test
...
class ContainerizedTest(db.Model):
    ...
    application = db.relationship(
        "GeneratedApplication", backref=db.backref("containerized_tests", lazy=True)
    )
...
class BatchAnalysis(db.Model):
    ...
    def get_progress_percentage(self) -> float:
        ...
        if self.total_applications == 0:
            return 0
        return round(
            (self.completed_applications + self.failed_applications)
...
class BatchJob(db.Model):
    ...
    tasks = db.relationship(
        "BatchTask", backref="job", lazy=True, cascade="all, delete-orphan"
    )
...
class BatchTask(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True)  # UUID string
    job_id = db.Column(
        db.String(36), db.ForeignKey("batch_jobs.id"), nullable=False, index=True
    )
...
class BatchWorker(db.Model):
    ...
    current_task_id = db.Column(
        db.String(36), db.ForeignKey("batch_tasks.id"), nullable=True
    )
