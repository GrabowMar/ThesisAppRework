...
class AppConfig:
    ...
    def __init__(self):
        ...
        self.DOCKER_TIMEOUT = self._safe_int_env(
            "DOCKER_TIMEOUT", AppDefaults.DOCKER_TIMEOUT
        )
        ...
        self.MODELS_BASE_DIR = Path(
            os.getenv("MODELS_BASE_DIR", self.BASE_DIR.parent / "models")
        )
        ...
        if self.SECRET_KEY == "your-secret-key-here" and not self.DEBUG:
            logging.warning(
                "SECURITY WARNING: FLASK_SECRET_KEY is not set in production!"
            )
...
class LoggingService:
    ...
    def setup(app: Flask) -> logging.Logger:
        ...
        # Setup handlers
        handlers = [
            LoggingService._create_file_handler(
                config.LOG_DIR / "app.log", log_level, detailed_formatter
            ),
            LoggingService._create_file_handler(
                config.LOG_DIR / "errors.log", logging.ERROR, detailed_formatter
            ),
        ...
        logging.info(
            f"Logging initialized. Level: {config.LOG_LEVEL}, Directory: {config.LOG_DIR}"
        )
        return root_logger
                ...
                try:
                    if (
                        ...
                        if len(parts) >= 2:
                            path = parts[1]
                            if any(
                                excluded in path for excluded in self.EXCLUDED_PATHS
                            ):
    ...
    def _create_file_handler(
        filename: Path, level: int, formatter: logging.Formatter
    ) -> logging.Handler:
...
class BaseService(ABC):
    ...
    def _log_operation_end(
        ...
        **context,
        ...
        self.logger.info(
            f"Completed {operation} [{operation_id}] - {status} in {duration_ms:.2f}ms {context_str}"
        )

        if not success:
            self.logger.warning(
                f"Operation {operation} [{operation_id}] failed after {duration_ms:.2f}ms"
            )
    ...
    def _safe_execute(self, operation: str, func, *args, **kwargs):
        ...
        try:
            ...
            self._log_operation_end(
                operation_id, operation, False, duration_ms, error=str(e)
            )
            self.logger.error(
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if request.path.startswith("/api/"):
                return jsonify(
                    {"success": False, "error": str(e), "message": "An error occurred"}
                ), 500
            raise
            ...
            if data.get(field_name):
                try:
                    ...
                    if isinstance(data[field_name], datetime):
                        data[f"{field_name}_formatted"] = data[field_name].strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        ...
                        try:
                            ...
                            data[f"{field_name}_formatted"] = dt.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
                        except (ValueError, TypeError):
                            data[f"{field_name}_formatted"] = data[
                                field_name
...
class DockerManager(BaseService):
        ...
        try:
            ...
            for docker_host in docker_hosts:
                try:
                    ...
                    client.ping()
                    self.logger.info(
                        f"Docker client created and verified (host: {docker_host})"
                    )
                    return client
                except Exception as e:
        ...
        try:
            ...
            return DockerStatus(
                ...
                health=health_info.get(
                    "Status", "healthy" if is_running else container_status
                ),
    ...
    def get_container_logs(
        self, model: str, app_num: int, container_type: str = "backend", tail: int = 100
    ) -> str:
        """Get container logs."""
        if not self.client:
    ...
    def execute_compose_command(
        ...
        try:
            ...
            base_commands = [["docker", "compose"], ["docker-compose"]]
            ...
            for base_cmd in base_commands:
                ...
                try:
                    ...
                    if (
                        result.returncode == 0
                        or "command not found" not in result.stderr.lower()
                    ):
    ...
    def start_containers(
        self, compose_path: str, model: str, app_num: int
    ) -> Dict[str, Any]:
        ...
        return self.execute_compose_command(
            compose_path, ["up", "-d", "--remove-orphans"], model, app_num, timeout=300
        )
    ...
    def stop_containers(
        ...
        return self.execute_compose_command(
            compose_path, ["down", "--timeout", "30"], model, app_num, timeout=180
        )
    ...
    def build_containers(
        self, compose_path: str, model: str, app_num: int, no_cache: bool = True
    ) -> Dict[str, Any]:
        ...
        return self.execute_compose_command(
            compose_path, command, model, app_num, timeout=900
        )
    ...
    def _stop_conflicting_containers(self, project_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if container_names:
                    self.logger.info(
                        f"Removed {len(container_names)} conflicting containers"
                    )
    ...
    def list_containers(
        self, name_filter: str = None, all_containers: bool = True
    ) -> List[Dict[str, Any]]:
...
class PortManager(CacheableService):
    ...
    def get_app_config(
        self, model_name: str, app_number: int
        ...
        for config in self.port_config:
            if (
                ...
                and config.get("app_number") == app_number
            ):
                self._set_cached(key, config)
                return config
        return None
    ...
    def get_app_ports(
        ...
        return {"backend": int(backend_port), "frontend": int(frontend_port)}
...
class TestingServiceClient(BaseService):
    ...
    def submit_security_analysis(
        self, model: str, app_num: int, tools: Optional[List[str]] = None
    ) -> str:
        ...
        try:
            ...
            response = session.post(
                f"{self.base_url}/api/security/tests", json=data, timeout=self.timeout
            )
    ...
    def submit_performance_test(
        self, model: str, app_num: int, target_url: str, users: int = 10
    ) -> str:
        ...
        try:
            ...
            response = session.post(
                f"{self.base_url}/api/performance/tests",
                json=data,
    ...
    def get_test_status(self, test_id: str, service_type: str) -> str:
        ...
        try:
            ...
            response = session.get(
                f"{self.base_url}/api/{endpoint}/tests/{test_id}/status"
            )
            response = session.get(
                f"{self.base_url}/api/{endpoint}/tests/{test_id}/status",
                timeout=self.timeout,
    ...
    def get_test_result(
        self, test_id: str, service_type: str
    ) -> Optional[Dict[str, Any]]:
        """Get test result."""
        try:
            session = self._get_session()
            ...
            endpoint = endpoint_map.get(service_type, "security")
            response = session.get(
                f"{self.base_url}/api/{endpoint}/tests/{test_id}/result"
            )
            response = session.get(
                f"{self.base_url}/api/{endpoint}/tests/{test_id}/result",
                timeout=self.timeout,
...
class OpenRouterAnalysisService(BaseService):
    ...
    async def analyze_code(
        self, model_slug: str, app_number: int, requirements: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def _get_application_source(
        self, model_slug: str, app_number: int
        ...
        try:
            ...
            if docker_compose.exists():
                source_files["docker_compose"] = docker_compose.read_text(
                    encoding="utf-8"
                )
        ...
        try:
            for file_path in directory.rglob("*"):
                ...
                if len(source_files) > 50:
                    self.logger.warning(
                        f"Too many files in {directory}, limiting to 50"
                    )
                    break
    ...
    def _build_analysis_prompt(
        self, source_code: Dict[str, Any], requirements: str = None
    ) -> str:
        """Build the analysis prompt for OpenRouter."""

        # Count files and estimate tokens
        total_files = sum(
            len(files) for files in source_code.values() if isinstance(files, dict)
        )
...
"""
            ...
            for analyzer_model in analyzer_models:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.warning(
                            f"Rate limited for {analyzer_model}, trying next..."
                        )
                        continue

                    else:
                        self.logger.warning(
                            f"API error {response.status_code} for {analyzer_model}: {response.text}"
                        )
                        continue

                except Exception as model_error:
                    self.logger.warning(
                        f"Failed to use {analyzer_model}: {model_error}"
                    )
                    continue
    ...
    def _extract_recommendations(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            # If we hit another major section, stop
            if line.startswith("#") and not any(
                keyword in line.lower() for keyword in rec_keywords
            ):
                in_rec_section = False
                continue
...
class ScanManager(BaseService):
                ...
                if scan.get("status") in terminal_statuses:
                    try:
                        end_time = datetime.fromisoformat(
                            scan.get("end_time", scan["start_time"])
                        )
                        if (
    ...
    def run_security_analysis(
        self, model: str, app_num: int, enabled_tools: dict
    ) -> dict:
        ...
        try:
            ...
            try:
                ...
                tools = []
                if any(
                    tool in enabled_tools and enabled_tools[tool]
                    for tool in ["bandit", "safety", "semgrep"]
                ):
                    tools.extend(["bandit", "safety", "pylint"])
                if any(
                    tool in enabled_tools and enabled_tools[tool]
                    for tool in ["eslint", "npm_audit", "retire"]
                ):
                ...
                test_id = self.testing_client.submit_security_analysis(
                    model, app_num, tools
                )
                ...
                self.logger.warning(
                    f"Containerized service failed, falling back to legacy: {containerized_error}"
                )
    ...
    def _run_legacy_security_analysis(
        self, model: str, app_num: int, enabled_tools: dict
    ) -> dict:
        """Fallback security analysis when containerized services are unavailable."""
        self.logger.warning(
            "Containerized security services unavailable, using mock analysis"
        )
        return {
            "success": True,
            "data": {
...
class ModelIntegrationService(CacheableService):
    ...
    def load_all_data(self) -> bool:
        ...
        try:
            if not self.app:
                self.logger.warning(
                    "No Flask app context available for database access"
                )
                return False
    ...
    def _load_port_configurations(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(port_configs)} port configurations from database"
            )
            return True
    ...
    def _load_model_capabilities(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded capabilities for {len(model_capabilities)} models from database"
            )
            return True
    ...
    def _generate_models_summary(self) -> bool:
        ...
        try:
            ...
            for model_name in sorted(models):
                ...
                models_list.append(
                    {"name": model_name, "color": color, "provider": provider}
                )
            ...
            self.logger.info(
                f"Generated summary for {len(models_list)} models from database"
            )
            return True
    ...
    def _integrate_model_data(self):
        ...
        for model_name, port_configs in port_configs_by_model.items():
            ...
            # Add summary data
            summary_models = {
                m["name"]: m for m in self._db_data["models_summary"].get("models", [])
            }
            ...
            if model_name in self._db_data["model_capabilities"]:
                ...
                model.supports_function_calling = caps.get(
        ...
        self.logger.info(
            f"Integrated data for {len(self.models_data)} models from database"
        )
...
class BatchTaskWorker:
    ...
    def execute_task(self, task: BatchTask) -> BatchTask:
        ...
        try:
            ...
            task.error = {"message": str(e), "category": type(e).__name__}
    ...
    def _run_analysis(self, analyzer, task: BatchTask) -> Dict[str, Any]:
        ...
        if hasattr(analyzer, "__class__") and "Mock" in analyzer.__class__.__name__:
            return {
                "status": "unavailable",
                "message": f"{task.analysis_type} service not installed",
            }
...
class BatchAnalysisService(BaseService):
    ...
    def create_job(
        ...
        job.progress["total"] = (
            len(models) * len(app_range["apps"]) * len(analysis_type_enums)
        )
    ...
    def start_job(self, job_id: str) -> bool:
        ...
        thread = threading.Thread(
            target=self._execute_job, args=(job_id,), name=f"BatchJob-{job_id[:8]}"
        )
...
def create_docker_blueprint() -> Blueprint:
    ...
    def docker_action():
        ...
        if not all([action, model, app_num]):
            return create_api_response(
                False, error="Missing required parameters", code=400
            )
        ...
        return create_api_response(
            success, message=message, code=200 if success else 500
        )
