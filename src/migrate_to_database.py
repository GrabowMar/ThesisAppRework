        ...
        for model_id, capabilities in models_data.items():
            try:
                ...
                model = ModelCapability(
                    ...
                    input_price_per_token=capabilities.get(
                        "input_price_per_token", 0.0
                    ),
                    output_price_per_token=capabilities.get(
...
def verify_migration(app):
    ...
    with app.app_context():
        ...
        if model_count > 0:
            ...
            print(
                f"   • Sample model: {sample_model.model_id} ({sample_model.provider})"
            )

        if port_count > 0:
            sample_port = PortConfiguration.query.first()
            print(
                f"   • Sample port: {sample_port.model} app {sample_port.app_num} -> {sample_port.frontend_port}/{sample_port.backend_port}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate data from JSON files to database"
    )
    parser.add_argument(
        "--force",
        action="store_true",
