...
def run_security_analysis_task(
    self,
    model_slug: str,
    app_number: int,
    tools: List[str],
    ...
    with app.app_context():
        try:
            ...
            update_task_progress(
                self.request.id, 0, 100, "Initializing security analysis"
            )
            ...
            with get_session() as session:
                if analysis_id:
                    ...
                    # Find application
                    app_record = (
                        session.query(GeneratedApplication)
                        .filter_by(model_slug=model_slug, app_number=app_number)
                        .first()
                    )

                    if not app_record:
                        raise ValueError(
                            f"Application {model_slug} app{app_number} not found"
                        )
...
def run_performance_test_task(
    ...
    with app.app_context():
        try:
            update_task_progress(
                self.request.id, 0, 100, "Initializing performance test"
            )
            ...
            return {"status": "completed", "test_id": test_id, "results": results}
...
def run_openrouter_analysis_task(
    self,
    model_slug: str,
    app_number: int,
    ...
    with app.app_context():
        try:
            ...
            # Run analysis
            results = openrouter_service.analyze_code(
                model_slug, app_number, requirements
            )
    ...
    with app.app_context():
        try:
            ...
            with get_session() as session:
                ...
                # Get tasks
                tasks = (
                    session.query(BatchTask).filter_by(batch_job_id=batch_job_id).all()
                )
...
def health_check_containers():
    ...
    with app.app_context():
        try:
            ...
            return {
                "status": "completed",
                "infrastructure_status": status.to_dict()
