...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = (
        os.environ.get("DATABASE_URL") or f"sqlite:///{db_path.absolute()}"
    )
...
class DatabasePopulator:
    ...
    def populate_models_from_json(self, capabilities_data: Dict[str, Any]) -> int:
        ...
        with self._lock:
            try:
                models_data: Optional[Dict[str, Any]] = self._extract_models_data(
                    capabilities_data
                )
                if not models_data:
                ...
                if errors:
                    self.logger.warning(
                        f"Errors during model population: {len(errors)} errors"
                    )
                    for error in errors[:5]:  # Log first 5 errors
                        self.logger.error(error)

                self.logger.info(
                    f"Successfully populated {models_created} model capabilities"
                )
    ...
    def _is_valid_model_data(self, model_data: Any) -> bool:
        ...
        return isinstance(model_data, dict) and model_data.get("model_id") is not None

    def _create_model_capability(
        ...
        model_capability.canonical_slug = model_data.get(
            "canonical_slug", model_data.get("model_id")
        )
        model_capability.provider = model_data.get("provider", "unknown")
        model_capability.model_name = model_data.get(
        ...
        model_capability.supports_function_calling = model_data.get(
        ...
        model_capability.supports_json_mode = model_data.get(
            "supports_json_mode", False
        )
        model_capability.input_price_per_token = float(
            model_data.get("pricing", {}).get("prompt_tokens", 0) or 0
        )
        model_capability.output_price_per_token = float(
            model_data.get("pricing", {}).get("completion_tokens", 0) or 0
        )
        model_capability.cost_efficiency = model_data.get(
            "performance_metrics", {}
        ).get("cost_efficiency", 0.0)
        model_capability.safety_score = model_data.get("quality_metrics", {}).get(
            "safety", 0.0
        )
        ...
        if hasattr(model_capability, "set_capabilities"):
            ...
            model_capability.capabilities_json = json.dumps(
    ...
    def populate_ports_from_json(self, port_data: List[Dict[str, Any]]) -> int:
        ...
        with self._lock:
            try:
                ...
                db.session.commit()
                self.logger.info(
                    f"Successfully populated {ports_created} port configurations"
                )
    ...
    def _create_port_configuration(self, port_entry: Dict[str, Any]) -> bool:
        ...
        if hasattr(port_config, "set_metadata"):
            port_config.set_metadata(
                {"app_type": port_entry.get("app_type", ""), "source": "initial_load"}
            )
        else:
...
def setup_logging(app: Flask) -> None:
    ...
    def after_request(response: Any) -> Any:
        ...
        if hasattr(g, "start_time"):
            ...
            app.logger.info(
                f"Request completed in {duration:.2f}ms - {response.status_code}"
            )
        return response
    ...
    try:
        ...
        with app.app_context():
            ...
            if model_capabilities:
                ...
                app.logger.info(
                    f"Generated models summary with {len(models_list)} models from database"
                )
            else:
        ...
        if not app.config.get("_ports_loaded", True):
            try:
                if PortConfiguration.query.count() == 0:
                    ...
                    if port_data:
                        app.logger.info(
                            "Populating database with port configurations..."
                        )
...
def create_app(config_name: Optional[str] = None) -> Flask:
    ...
    app.logger.info(
        "Flask application created successfully with consolidated HTMX template system"
    )
    return app
