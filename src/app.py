...
try:
    ...
    from .models import GeneratedApplication, ModelCapability, PortConfiguration
...
class DatabasePopulator:
    ...
    def populate_models_from_json(self, capabilities_data: dict[str, Any]) -> int:
        ...
        with self._lock:
            try:
                models_data: dict[str, Any] | None = self._extract_models_data(capabilities_data)
                if not models_data:
    ...
    def _extract_models_data(self, capabilities_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _create_model_capability(self, model_id: str, model_data: dict[str, Any]) -> bool:
        ...
        existing: ModelCapability | None = ModelCapability.query.filter_by(
    ...
    def _create_port_configuration(self, port_entry: dict[str, Any]) -> bool:
        ...
        frontend_port: int | None = port_entry.get('frontend_port')
        backend_port: int | None = port_entry.get('backend_port')

        if not frontend_port or not backend_port:
            return False

        # Check if port configuration already exists
        existing: PortConfiguration | None = PortConfiguration.query.filter_by(
...
def create_app(config_name: str | None = None) -> Flask:
    ...
    def inject_template_vars() -> dict[str, str | bool]:
