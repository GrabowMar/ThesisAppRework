    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = (time.time() - start_time) * 1000
                logger.error(
                    f"Failed {op_name} [{request_id}] after {duration:.2f}ms: {e}"
                )
                raise
        ...
        try:
            if ResponseHandler.is_htmx_request():
                ...
                if partial_type:
                    ...
                    template_base = template_name.replace(".html", "").replace(
                        "pages/", ""
                    )
                    result = render_template(
                        f"partials/{template_base}_{partial_type}.html", **context
                    )
                else:
                    ...
                    result = render_template(
                        f"partials/{template_base}.html", **context
                    )
            else:
            ...
            duration = (time.time() - start_time) * 1000
            logger.debug(
                f"Template {clean_template} rendered successfully in {duration:.2f}ms"
            )
            ...
            duration = (time.time() - start_time) * 1000
            logger.error(
                f"Template rendering failed for {template_name} after {duration:.2f}ms: {e}"
            )
            raise
...
class DockerCache:
    ...
    def get_all_containers_cached(self, docker_manager) -> List[Any]:
        ...
        with self._lock:
            ...
            if (
                "all_containers" in self._cache
                and "all_containers" in self._cache_timestamp
                and now - self._cache_timestamp["all_containers"] < self._cache_duration
            ):
            ...
            try:
                if docker_manager and docker_manager.client:
                    ...
                    logger.info(
                        f"Docker cache refreshed: {len(containers)} containers found"
                    )
                    return containers
                else:
                    return []
    ...
    def get_container_status_cached(self, docker_manager, container_name: str) -> str:
        ...
        with self._lock:
            ...
            if (
                ...
                and now - self._cache_timestamp[cache_key] < self._cache_duration
            ):
    ...
    def bulk_get_container_statuses(
        self, docker_manager, container_names: List[str]
    ) -> Dict[str, str]:
        ...
        with self._lock:
            ...
            container_map = {
                container.name: container.status for container in all_containers
            }
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "last_refresh": max(self._cache_timestamp.values())
...
class AppDataProvider:
    ...
    APP_TYPES = {
        ...
        20: "Mapping App",
        21: "Recipe Manager",
        22: "Learning Platform",
        23: "Finance Tracker",
        24: "Networking Tool",
        25: "Health Monitor",
        26: "Environment Tracker",
        27: "Team Management",
        28: "Art Portfolio",
        29: "Event Planner",
    ...
    def get_app_info(model: str, app_num: int) -> Dict[str, Any]:
        ...
        try:
            app = GeneratedApplication.query.filter_by(
                model_slug=model.replace("-", "_"), app_number=app_num
            ).first()
            ...
            if app:
                return {
                    ...
                    or AppDataProvider.APP_TYPES.get(app_num, "unknown"),
    ...
    def get_port_config(model: str, app_num: int) -> Dict[str, int]:
        ...
        try:
            ...
            if config:
                logger.debug(
                    f"Found port config in DB for {model}/app{app_num}: frontend={config.frontend_port}, backend={config.backend_port}"
                )
                return {
            ...
            config = PortConfiguration.query.filter_by(
                model=db_format, app_num=app_num
            ).first()

            if config:
                logger.debug(
                    f"Found port config in DB for {db_format}/app{app_num} (original: {model}): frontend={config.frontend_port}, backend={config.backend_port}"
                )
                return {
                    "backend_port": config.backend_port,
                    "frontend_port": config.frontend_port,
            ...
            for model_variant in model_variations:
                config = PortConfiguration.query.filter_by(
                    model=model_variant, app_num=app_num
                ).first()

                if config:
                    logger.debug(
                        f"Found port config in DB for {model_variant}/app{app_num} (original: {model}): frontend={config.frontend_port}, backend={config.backend_port}"
                    )
                    return {
                        "backend_port": config.backend_port,
                        "frontend_port": config.frontend_port,
            ...
            logger.warning(
                f"Error querying port config from DB for {model}/app{app_num}: {e}"
            )
    ...
    def get_container_statuses(model: str, app_num: int) -> Dict[str, str]:
        ...
        try:
            ...
            statuses = _docker_cache.bulk_get_container_statuses(
                docker_manager, container_names_list
            )
            ...
            logger.error(
                f"Error getting container statuses for {model}/app{app_num}: {e}"
            )
                ...
                apps.append(
                    {
                        ...
                        "status": app.container_status or "unknown",
                        "app_type": app.app_type
                        or AppDataProvider.APP_TYPES.get(app.app_number, "unknown"),
                        "provider": app.provider or "unknown",
                    }
                )
        ...
        if docker_manager:
            try:
                ...
                if backend_status == "running" and frontend_status == "running":
                    status = "running"
                elif backend_status in ["exited", "dead"] or frontend_status in [
                    "exited",
                    "dead",
                ]:
                    status = "error"
        ...
        try:
            ...
            if docker_manager:
                for app_num in range(1, 31):
                    try:
                        statuses = AppDataProvider.get_container_statuses(
                            model_slug, app_num
                        )
                        ...
                        elif backend in ["exited", "dead"] or frontend in [
                            "exited",
                            "dead",
                        ]:
...
class DockerOperations:
    ...
    def execute_action(action: str, model: str, app_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = str(e)
            if (
                "Nie można odnaleźć określonego pliku" in error_msg
                or "dockerDesktopLinuxEngine" in error_msg
            ):
                error_msg = "Docker Desktop is not running. Please start Docker Desktop and try again."
            elif "unable to get image" in error_msg:
                error_msg = (
                    "Docker images not built. Please build the containers first."
                )
    ...
    @staticmethod
    def bulk_action(
        action: str, apps: List[Tuple[str, int]], max_workers: int = 2
    ) -> Dict[str, Any]:
...
testing_bp = Blueprint(
    "testing", __name__, url_prefix="/testing"
...
def test_creation_page():
    ...
    try:
        ...
        return ResponseHandler.render_response(
            "pages/test_creation.html",
...
def test_dashboard():
    ...
    try:
        ...
        recent_tests = (
            BatchJob.query.order_by(BatchJob.created_at.desc()).limit(20).all()
        )
    ...
    try:
        ...
        if batch_job:
            ...
            test_data = {
                ...
                "status": containerized_test.status.value
                if containerized_test.status
                else "unknown",
                "type": "containerized",
        ...
        return ResponseHandler.render_response(
            "pages/test_results.html", test_data=test_data
        )
...
def validate_configuration():
    ...
    try:
        ...
        validation_schemas = {
            ...
            "zap": {
                "scan_type": {
                    "required": True,
                    "type": "string",
                    "options": ["baseline", "active", "api", "full"],
                },
        ...
        return jsonify(
            {"valid": len(errors) == 0, "errors": errors, "tool_name": tool_name}
        )
...
def get_infrastructure_status():
    ...
    try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        openrouter_enabled = response_data.get(
        ...
        data = {"services": services, "overall_health": overall_health}
        ...
        return render_template(
            "partials/infrastructure_status_error.html", error=str(e)
        )
...
def get_test_stats():
    ...
    try:
        ...
        success_rate = (
            round((completed_jobs / max(total_jobs, 1)) * 100, 1)
            if total_jobs > 0
            else 100
        )
...
def get_test_jobs():
    ...
    try:
        ...
        if status_filter == "running" or not status_filter:
            ...
            batch_jobs = BatchJob.query.filter(
                BatchJob.status == JobStatus.RUNNING
            ).all()
            ...
            containerized_tests = (
                ...
                .order_by(ContainerizedTest.submitted_at.desc())
                .limit(limit)
                .all()
            )
        ...
        return render_template(
            "partials/active_tests_unified.html", active_tests=active_tests
        )
    except Exception as e:
...
def get_containerized_test_status(test_id):
    ...
    try:
        ...
        status_info = {
            "status": test.status.value,
            "submitted_at": test.submitted_at.strftime("%Y-%m-%d %H:%M:%S")
            if test.submitted_at
            else None,
            "started_at": test.started_at.strftime("%Y-%m-%d %H:%M:%S")
            if test.started_at
            else None,
            "completed_at": test.completed_at.strftime("%Y-%m-%d %H:%M:%S")
        ...
        return render_template(
            "partials/test_status_info.html", status=status_info, test=test
        )
    except Exception as e:
...
def get_containerized_test_results(test_id):
    ...
    try:
        ...
        return render_template(
            "partials/containerized_test_results_modal.html", test=test, results=results
        )
    except Exception as e:
...
def get_new_test_form():
    ...
    try:
        ...
        return render_template(
            "partials/new_test_modal_fixed.html", models=models, test_types=test_types
        )
    except Exception as e:
...
def submit_test():
    ...
    try:
        ...
        logger.info(
            f"Test submission: type={test_type}, model={model_slug}, app={app_number}, containerized={use_containerized}"
        )

        if not all([test_type, model_slug, app_number]):
            return render_template(
                "partials/test_submission_error.html", error="Missing required fields"
            )
        ...
        try:
            ...
            return render_template(
                "partials/test_submission_error.html",
        ...
        if test_type == "security":
            logger.info(
                f"Processing security test, use_containerized={use_containerized}"
            )
            logger.info(
                f"Analyzer has containerized_testing_client: {hasattr(analyzer, 'containerized_testing_client')}"
            )
            if use_containerized and hasattr(analyzer, "containerized_testing_client"):
                ...
                try:
                    ...
                    response = requests.post(
                        "http://localhost:8001/tests", json=request_data, timeout=30
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if response_data.get("success"):
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Created ContainerizedTest record for test_id: {test_id}"
                                )

                            except Exception as e:
                                logger.error(
                                    f"Failed to create ContainerizedTest record: {e}"
                                )
                                db.session.rollback()  # Roll back the transaction on error
                                # Don't fail the test submission over this
                        else:
                            logger.error(
                                f"Test submission failed: {response_data.get('error')}"
                            )
                    else:
                        logger.error(
            ...
            if use_containerized and hasattr(analyzer, "containerized_testing_client"):
                ...
                target_url = request.form.get("target_url", "http://localhost:3000")

                result = analyzer.containerized_testing_client.run_performance_test(
                    target_url=target_url, users=concurrent_users, duration=duration
                )
            else:
                return render_template(
                    "partials/test_submission_error.html",
        ...
        if result:
            ...
            return render_template(
                "partials/test_submission_success_fixed.html",
                test_id=test_id,
                test_type=test_type,
                model_slug=model_slug,
                app_number=app_number,
        ...
        return render_template("partials/test_submission_error.html", error=str(e))
...
def start_test_job(job_id):
    ...
    try:
        ...
        if not job:
            return render_template(
                "partials/job_action_error.html", error="Job not found", job_id=job_id
            )

        if job.status != JobStatus.PENDING:
            return render_template(
                "partials/job_action_error.html",
        ...
        return render_template(
            "partials/job_action_success.html", action="started", job_id=job_id
        )

    except Exception as e:
        logger.error(f"Error starting job {job_id}: {e}")
        return render_template(
            "partials/job_action_error.html", error=str(e), job_id=job_id
        )
...
def stop_test_job(job_id):
    ...
    try:
        ...
        return render_template(
            "partials/job_action_success.html", action="stopped", job_id=job_id
        )
...
def get_job_progress(job_id):
    ...
    try:
        ...
        if not job:
            return render_template(
                "partials/job_progress_error.html", error="Job not found", job_id=job_id
            )
        ...
        return render_template(
            "partials/job_progress.html", job=job, progress_percent=progress_percent
        )

    except Exception as e:
        logger.error(f"Error getting job progress {job_id}: {e}")
        return render_template(
            "partials/job_progress_error.html", error=str(e), job_id=job_id
        )
...
def get_job_results(job_id):
    ...
    try:
        ...
        if not job:
            return render_template(
                "partials/job_results_error.html", error="Job not found", job_id=job_id
            )
        ...
        return render_template(
            "partials/job_results_error.html", error=str(e), job_id=job_id
        )
...
def get_job_logs(job_id):
    ...
    try:
        ...
        if not job:
            return render_template(
                "partials/job_logs_error.html", error="Job not found", job_id=job_id
            )
        ...
        if job.error_details_json:
            ...
            logs = [
                ...
                else f"INFO: Job status - {job.status.name}"
            ]

        return render_template("partials/job_logs.html", job=job, logs=logs)

    except Exception as e:
        logger.error(f"Error getting job logs {job_id}: {e}")
        return render_template(
            "partials/job_logs_error.html", error=str(e), job_id=job_id
        )
    ...
    try:
        ...
        if result["success"]:
            return ResponseHandler.success_response(
                message=f"Started containers for {model_slug}/app{app_num}", data=result
            )
        else:
            return ResponseHandler.error_response(
                result.get("error", "Start operation failed")
            )
    except Exception as e:
...
def container_stop(model_slug: str, app_num: int):
    ...
    try:
        ...
        if result["success"]:
            return ResponseHandler.success_response(
                message=f"Stopped containers for {model_slug}/app{app_num}", data=result
            )
        else:
            return ResponseHandler.error_response(
                result.get("error", "Stop operation failed")
            )
    except Exception as e:
...
def container_restart(model_slug: str, app_num: int):
    ...
    try:
        ...
        if result["success"]:
            ...
            return ResponseHandler.error_response(
                result.get("error", "Restart operation failed")
            )
    except Exception as e:
        ...
        if docker_manager and models:
            ...
            for model in sample_models:
                for app_num in range(1, 6):
                    try:
                        statuses = AppDataProvider.get_container_statuses(
                            model.canonical_slug, app_num
                        )
                        checked_containers += 1

                        if (
                            statuses.get("backend") == "running"
                            and statuses.get("frontend") == "running"
                        ):
                            stats["running_containers"] += 1
                        elif statuses.get("backend") in [
                            "exited",
                            "dead",
                        ] or statuses.get("frontend") in ["exited", "dead"]:
                            stats["error_containers"] += 1
                    except Exception as e:
                        logger.debug(
                            f"Container status check failed for {model.canonical_slug}/app{app_num}: {e}"
                        )
                        pass
            ...
            if len(sample_models) > 0:
                ...
                stats["running_containers"] = int(
                    stats["running_containers"] * scale_factor
                )
                stats["error_containers"] = int(
                    stats["error_containers"] * scale_factor
                )
                logger.debug(
                    f"Checked {checked_containers} containers, scaled estimates: running={stats['running_containers']}, errors={stats['error_containers']}"
                )
        else:
            logger.warning(
                "Docker manager unavailable or no models found for dashboard"
            )

        logger.info(
            f"Dashboard loaded successfully with {stats['total_models']} models"
        )
...
def app_details(model: str, app_num: int):
    ...
    try:
        ...
        return redirect(
            url_for("main.app_overview", model=decoded_model, app_num=app_num)
        )

    except Exception as e:
        logger.error(f"Error redirecting to app overview: {e}")
        return ResponseHandler.error_response(
            f"Failed to redirect to app overview: {str(e)}"
        )
...
def app_overview(model: str, app_num: int):
    ...
    try:
        ...
        container_statuses = AppDataProvider.get_container_statuses(
            decoded_model, app_num
        )
        ...
        generated_app = GeneratedApplication.query.filter_by(
            model_slug=decoded_model, app_number=app_num
        ).first()
            ...
            context["app_info"].update(
                {
                    ...
                    "created_date": generated_app.created_at.strftime("%Y-%m-%d")
                    if generated_app.created_at
                    else None,
                    "last_modified": generated_app.updated_at.strftime("%Y-%m-%d")
                    if generated_app.updated_at
                    else None,
                }
            )
...
def app_docker(model: str, app_num: int):
    ...
    try:
        ...
        return ResponseHandler.error_response(
            f"Failed to load app docker page: {str(e)}"
        )
...
def app_analysis(model: str, app_num: int):
    ...
    try:
        ...
        return ResponseHandler.error_response(
            f"Failed to load app analysis page: {str(e)}"
        )
...
def app_performance(model: str, app_num: int):
    ...
    try:
        ...
        try:
            ...
            context["performance_available"] = (
                False  # Will be enabled when containers are available
            )
        ...
        return ResponseHandler.error_response(
            f"Failed to load app performance page: {str(e)}"
        )
...
def app_files(model: str, app_num: int):
    ...
    try:
        ...
        return ResponseHandler.error_response(
            f"Failed to load app files page: {str(e)}"
        )
...
def app_tests(model: str, app_num: int):
    ...
    try:
        ...
        return ResponseHandler.error_response(
            f"Failed to load app tests page: {str(e)}"
        )
    ...
    try:
        ...
        app_stats = (
            db.session.query(
                ...
                func.count(
                    func.nullif(GeneratedApplication.generation_status, "pending")
                ).label("generated_apps"),
                func.count(
                    func.nullif(GeneratedApplication.container_status, "stopped")
        ...
        for model in models:
            ...
            if model.canonical_slug in app_stats_dict:
                ...
                model_data.update(
                    {"total_apps": 0, "generated_apps": 0, "running_containers": 0}
                )
        ...
        summary_stats = {
            ...
            "total_running": sum(
                m.get("running_containers", 0) for m in enhanced_models
            ),
            "vision_models": sum(
                1 for m in enhanced_models if m.get("supports_vision")
            ),
            "function_calling_models": sum(
                1 for m in enhanced_models if m.get("supports_function_calling")
            ),
            "free_models": sum(1 for m in enhanced_models if m.get("is_free")),
            "avg_context_window": sum(
                m.get("context_window", 0) for m in enhanced_models
            )
            // max(1, len(enhanced_models)),
        }
...
def export_models():
    ...
    try:
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=models_export_{timestamp}.json"
        )
        return response
    ...
    try:
        ...
        if not model:
            ...
            for variant in model_variations:
                ...
                if model:
                    logger.debug(
                        f"Found model using variant: {variant} (original: {decoded_model_slug})"
                    )
                    break

        if not model:
            logger.warning(
                f"Model not found for slug: {decoded_model_slug} (sanitized: {sanitized_model})"
            )
            return ResponseHandler.error_response("Model not found", 404)
        ...
        apps_data = []
        for app_num in range(1, 31):
            app_data = AppDataProvider.get_app_for_dashboard(
                model_slug_for_lookup, app_num
            )
            apps_data.append(app_data)

        # Return JSON data instead of partial template
        return ResponseHandler.success_response(
            data={"apps": apps_data, "model_slug": model_slug_for_lookup}
        )
    ...
    try:
        ...
        for model in models:
            ...
            model_data = {
                ...
                "stats": AppDataProvider.get_model_dashboard_stats(
...
def api_model_stats(model_slug: str):
    ...
    try:
        ...
        model = ModelCapability.query.filter_by(
            canonical_slug=decoded_model_slug
        ).first()
        if not model:
            ...
            try:
                ...
                for app_num in range(1, 31):
                    try:
                        ...
                        if not backend_name:
                            backend_name = (
                                f"{container_model_name}_app{app_num}_backend"
                            )
                        if not frontend_name:
                            frontend_name = (
                                f"{container_model_name}_app{app_num}_frontend"
                            )
                        ...
                        logger.debug(
                            f"Error checking app {app_num} for model {model_slug}: {app_error}"
                        )
        ...
        return jsonify({"running": 0, "stopped": 0, "error": 0, "total": 60}), 500
...
def api_model_details(model_slug: str):
    ...
    try:
        ...
        capabilities = (
            model.get_capabilities() if hasattr(model, "get_capabilities") else {}
        )
        ...
        if ResponseHandler.is_htmx_request():
            ...
            if docker_manager:
                try:
                    ...
                    logger.warning(
                        f"Failed to get container stats for {decoded_model_slug}: {e}"
                    )
                    stopped_count = 30
            else:
                stopped_count = 30
            ...
            model_data = {
                ...
                "context_length_formatted": f"{model.max_context_length:,}"
...
def api_model_start_all(model_slug: str):
    ...
    try:
        ...
        return ResponseHandler.success_response(
            data=result, message=f"Started all apps for {model.model_name}"
        )
...
def api_model_stop_all(model_slug: str):
    ...
    try:
        ...
        return ResponseHandler.success_response(
            data=result, message=f"Stopped all apps for {model.model_name}"
        )
...
def api_model_restart_all(model_slug: str):
    ...
    try:
        ...
        return ResponseHandler.success_response(
            data=result, message=f"Restarted all apps for {model.model_name}"
        )
...
def api_model_analyze_all(model_slug: str):
    ...
    try:
        ...
        for app_num in range(1, 31):
            try:
                job_id = scan_manager.queue_analysis(
                    model_slug, app_num, "backend_security"
                )
                if job_id:
                    analysis_jobs.append(job_id)
            except Exception as job_error:
                logger.warning(
                    f"Failed to queue analysis for {model_slug} app {app_num}: {job_error}"
                )
        ...
        if result["success"]:
            ...
            if ResponseHandler.is_htmx_request():
                ...
                if "dashboard" in referrer:
                    ...
                    return render_template(
                        "partials/app_status_badge.html",
                        statuses=statuses,
                        model=decoded_model,
                        app_num=app_num,
                    )
            ...
            if ResponseHandler.is_htmx_request():
                ...
                if "dashboard" in referrer:
                    ...
                    return render_template(
                        "partials/app_table_row_error.html",
                        app=app_data,
                        model_slug=decoded_model,
                        show_error=True,
                    )
    ...
    try:
        ...
        if ResponseHandler.is_htmx_request():
            ...
            if "dashboard" in referrer or "app_details" in referrer:
                # Full status page
                return render_template(
                    "partials/container_status.html",
                    statuses=statuses,
                    model=decoded_model,
                    app_num=app_num,
                )
...
def get_app_files(model: str, app_num: int):
    ...
    try:
        ...
        if not app_dir.exists():
            return ResponseHandler.error_response(
                "Application directory not found", 404
            )
        ...
        return ResponseHandler.success_response(
            data={"files": files, "total_files": total_files}
        )
...
def api_get_models():
    ...
    try:
        ...
        return ResponseHandler.success_response(
            {"models": models_data, "total": len(models_data)}
        )
...
def get_app_info(model: str, app_num: int):
    ...
    try:
        ...
        model_capability = ModelCapability.query.filter_by(
            model_id=decoded_model
        ).first()
...
def api_start_app(model: str, app_num: int):
    ...
    try:
        ...
        if result["success"]:
            ...
            return ResponseHandler.error_response(
                f"Failed to start app: {result.get('error', 'Unknown error')}"
            )
...
def api_stop_app(model: str, app_num: int):
    ...
    try:
        ...
        if result["success"]:
            ...
            return ResponseHandler.error_response(
                f"Failed to stop app: {result.get('error', 'Unknown error')}"
            )
...
def api_restart_app(model: str, app_num: int):
    ...
    try:
        ...
        if result["success"]:
            ...
            return ResponseHandler.error_response(
                f"Failed to restart app: {result.get('error', 'Unknown error')}"
            )
...
def export_models_data():
    ...
    try:
        ...
        if format_type == "csv":
            ...
            response.headers["Content-Disposition"] = (
                f"attachment; filename=models_export_{timestamp}.csv"
            )
            return response
        else:
            # JSON format
            export_result = {
                "metadata": {
                    ...
                    "total_models": len(export_data),
                    "filters_applied": {"search": search, "provider": provider},
                },
                "models": export_data,
            }
            ...
            if date_dirs:
                ...
                if daily_summary_file.exists():
                    ...
                    successful_calls = sum(
                        1 for call in daily_data if call.get("success", False)
                    )
                    total_duration = 0
                    duration_count = 0
                    ...
                    model_subdirs = [
                        ...
                        if d.is_dir() and not d.name.startswith("daily_")
                    ]
            ...
            if date_dirs:
                ...
                if daily_summary_file.exists():
                    ...
                    for call in daily_data[:15]:  # Get last 15 calls
                        model_display = (
                            call.get("model", "").split("/")[-1].replace(":free", "")
                        )
...
def create_batch_job():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            context = {
                "available_models": [
                    {"slug": m.canonical_slug, "name": m.model_name, "apps_count": 30}
                    for m in models
                ],
                "analysis_types": [
                    {
                        "value": "security_combined",
                        "name": "Security Comprehensive",
                        "description": "Full security analysis (Backend + Frontend)",
                    },
                    {
                        "value": "security_backend",
                        "name": "Security Backend",
                        "description": "Bandit, Safety, Semgrep",
                    },
                    {
                        "value": "security_frontend",
                        "name": "Security Frontend",
                        "description": "ESLint Security, Retire.js",
                    },
                    {
                        "value": "performance",
        ...
        try:
            ...
            job_data = {
                ...
                "max_concurrent_tasks": int(
                    request.form.get("max_concurrent_tasks", 4)
                ),
            ...
            if not target_models:
                return ResponseHandler.error_response(
                    "At least one target model must be selected"
                )
            ...
            job_data["security_config"] = _extract_tool_config(
                request.form, "security_config"
            )
            job_data["frontend_config"] = _extract_tool_config(
                request.form, "frontend_config"
            )
            job_data["performance_config"] = _extract_tool_config(
                request.form, "performance_config"
            )
            ...
            if ResponseHandler.is_htmx_request():
                return render_template(
                    "partials/batch_job_create_success.html", job=job
                )

            flash(
                f'Batch job "{job.name}" created successfully ({job.total_tasks} tasks)',
                "success",
            )
...
def run_performance_test(model, app_num):
    ...
    try:
        ...
        if results["success"]:
            return render_template(
                "partials/performance_test_success.html",
                results=results.get("data", {}),
            )
        else:
            return render_template(
                "partials/performance_test_failed.html",
...
def _run_containerized_security_analysis(model, app_num):
    ...
    try:
        ...
        if not scan_manager or not hasattr(scan_manager, "testing_client"):
            return jsonify(
                {"error": "Containerized testing service not available"}
            ), 503

        # Submit analysis
        test_id = scan_manager.testing_client.submit_security_analysis(
            model, app_num, tools
        )

        return render_template(
            "partials/security_analysis_submitted.html",
            test_id=test_id,
            tools=tools,
            model=model,
            app_num=app_num,
        ...
        return render_template(
            "partials/security_analysis_warning.html", error=str(e)
        ) + _run_legacy_security_analysis(model, app_num)
...
def _run_legacy_security_analysis(model, app_num):
    ...
    try:
        ...
        if not app_dir.exists():
            return render_template(
                "partials/application_not_found.html", model=model, app_num=app_num
            )

        # Run comprehensive CLI analysis
        if use_all_tools:
            analysis_results = cli_analyzer.run_full_analysis(
                model=model, app_num=app_num, use_all_tools=True, save_to_db=True
            )
        else:
            ...
            if include_quality:
                categories.extend(
                    [ToolCategory.BACKEND_QUALITY, ToolCategory.FRONTEND_QUALITY]
                )

            analysis_results = cli_analyzer.run_analysis(
                model=model, app_num=app_num, categories=categories, use_all_tools=False
            )

        # Check if analysis completed successfully
        if analysis_results and not any(
            "error" in result
            ...
            return render_template(
                "partials/security_analysis_completed.html",
            ...
            return render_template(
                "partials/security_analysis_failed.html",
...
def get_analysis_results(model, app_num):
    ...
    try:
        ...
        analysis = (
            ...
            .filter(
                GeneratedApplication.model_slug == model,
        ...
        return render_template(
            "partials/analysis_results_display.html",
            total_issues=total_issues,
            results_html="".join(results_html),
...
def api_batch_stats():
    ...
    try:
        ...
        if client_ip in _batch_stats_clients:
            if (
                current_time - _batch_stats_clients[client_ip] < 0.001
            ):  # 1ms cooldown - only stops 1000+ req/sec
                logger.debug(
                    f"Extreme rate limit hit for batch/stats endpoint from {client_ip}"
                )
        ...
        expired_ips = [
            ip
            for ip, timestamp in _batch_stats_clients.items()
    ...
    try:
        ...
        job_config = {
            "operation_type": request.form.get(
                "operation_type"
            ),  # Fixed: use operation_type instead of test_type
            "job_name": request.form.get("job_name"),
            "description": request.form.get("description", ""),
            "tools": request.form.getlist("tools"),
            "target_selection": request.form.get(
                "target_selection", "models"
            ),  # Fixed: use target_selection
        ...
        if result["success"]:
            return ResponseHandler.success_response(
                data={"job_id": result["job_id"]}, message=result["message"]
            )
        else:
...
def testing_api_jobs():
    ...
    try:
        ...
        for job in jobs_db:
            ...
            if job.created_at:
                job_dict["created_at_display"] = job.created_at.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )

            # Add models and test types for filtering
            job_dict["models_display"] = ", ".join(
                job.get_models()[:3]
            )  # First 3 models
        ...
        if test_type_filter and test_type_filter != "all":
            jobs = [
                job
                for job in jobs
                if test_type_filter in job.get("test_types_display", "")
            ]

        if model_filter and model_filter != "all":
            jobs = [
                job for job in jobs if model_filter in job.get("models_display", "")
            ]
        ...
        if request.headers.get("HX-Request"):
            return render_template(
                "partials/batch_jobs_list.html", jobs=[], error=str(e)
            )
        else:
            return jsonify({"success": False, "error": str(e)}), 500
...
def testing_api_export():
    ...
    try:
        ...
        if format_type == "csv":
            ...
            # Write header
            writer.writerow(
                ["ID", "Name", "Status", "Created At", "Completed At", "Duration"]
            )
...
def testing_api_models():
    ...
    try:
            ...
            if model_service and hasattr(model_service, "get_all_models"):
                ...
                for model in all_models:
                    ...
                    if hasattr(model, "__dict__"):
                        model_id = getattr(
                            model, "model_name", getattr(model, "name", "unknown")
                        )
        ...
        return jsonify(
            {"success": True, "models": models_data, "total_count": len(models_data)}
        )
    ...
    try:
        ...
        try:
            ...
            if port_config_file.exists() and models_dir.exists():
                    ...
                    if app_dir.exists() and docker_compose_path.exists():
                        ...
                        class RealApp:
                            def __init__(self, config, app_dir):
                                self.id = (
                                    f"{config['model_name']}_app{config['app_number']}"
                                )
                ...
                logger.info(
                    f"Found {len(applications)} real applications with docker-compose files"
                )

            else:
                logger.warning("Port config or models directory not found")
            ...
            for app in applications:
                ...
                if model_slug not in seen_models:
                    ...
                    if "_" in model_slug:
                        ...
                        display_name = (
                            model_slug.replace("_", " ").replace("-", " ").title()
                        )
                    else:
                        provider = "unknown"
        ...
        logger.info(
            f"Returning {len(applications)} applications and {len(models_data)} models to new test form"
        )

        return render_template(
            "partials/new_test_modal_fixed.html",
        ...
        return render_template(
            "partials/error_message.html", error="Could not load test form"
        ), 500
...
def testing_app_details():
    ...
    try:
        ...
        if "_app" in app_id:
            ...
            return (
                '<div class="alert alert-warning">Invalid application ID format</div>'
            )
        ...
        if not app_dir.exists():
            return (
                '<div class="alert alert-warning">Application directory not found</div>'
            )
        ...
        try:
            ...
            if port_config_file.exists():
                ...
                for config in port_config:
                    if config["model_name"] == model_slug and config[
                        "app_number"
                    ] == int(app_number):
                        port_info = config
                        break
        ...
        if has_compose:
            try:
                ...
                if docker_manager:
                    ...
                    project_name = (
                        f"{model_slug.lower().replace('_', '')}-app{app_number}"
                    )
        ...
        file_counts = {
            "backend": len(list(backend_dir.glob("**/*")))
            if backend_dir.exists()
            else 0,
            "frontend": len(list(frontend_dir.glob("**/*")))
...
def testing_api_infrastructure_management(action):
    ...
    try:
        ...
        testing_infra_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "testing-infrastructure"
        )
        manage_script = os.path.join(testing_infra_path, "manage.py")

        if not os.path.exists(manage_script):
            return jsonify(
                {"success": False, "error": "Testing infrastructure not available"}
            ), 404

        # Execute the management command
        cmd = ["python", manage_script, action]
        result = subprocess.run(
            cmd, cwd=testing_infra_path, capture_output=True, text=True, timeout=120
        )
...
def testing_api_infrastructure_logs():
    ...
    try:
        ...
        result = subprocess.run(
            cmd, cwd=testing_infra_path, capture_output=True, text=True, timeout=30
        )

        if result.returncode == 0:
            logs = result.stdout.split("\n")
            return jsonify(
                {"success": True, "logs": logs, "timestamp": datetime.now().isoformat()}
            )
        else:
            return jsonify(
                {
            ...
            export_data.append(
                {
                    ...
                    "Total Issues": job.total_issues
                    if hasattr(job, "total_issues")
                    else 0,
...
def testing_api_create():
    ...
    try:
        ...
        if request.is_json or request.content_type == "application/json":
            ...
            logger.error(
                f"No supported content type. Content-Type: {request.content_type}"
            )
            return jsonify(
                {
                    "success": False,
...
def _handle_comprehensive_test_creation(data):
    ...
    if not app_id:
        return jsonify(
            {"success": False, "error": "Please select an application to test"}
        ), 400
    ...
    new_job = BatchJob(
        ...
        description=data.get(
            "description", f"Comprehensive security test with {', '.join(test_types)}"
        ),
        status=JobStatus.PENDING,
...
def _handle_legacy_test_creation(data):
    ...
    app_range = {"apps": apps, "include_all": len(apps) == 0}
    new_job.set_app_range(app_range)
        ...
        if port_config_file.exists() and models_dir.exists():
            ...
            for config in port_config:
                ...
                if config_app_id == app_id:
                    ...
                    if app_dir.exists() and docker_compose_path.exists():
                        provider = (
                            model_name.split("_")[0] if "_" in model_name else "unknown"
                        )
    ...
    if "security" in test_types:
        ...
        options["security_config"] = {
            ...
            "backend_tools": [
                ...
                if tool in ["bandit", "safety", "pylint", "semgrep"]
            ],
            "frontend_tools": [
                tool
                for tool in security_tools
                if tool in ["eslint", "retire", "npm-audit", "secrets"]
            ],
        }
...
def _process_enhanced_tool_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if test_type == "security_analysis":
        ...
        if "bandit" in data.get("tools", []):
            security_config["bandit"] = {
                ...
                "exclude_paths": data.get("bandit_exclude_paths", "").split(",")
                if data.get("bandit_exclude_paths")
                else [],
                "include_tests": data.get("bandit_include_tests", "").split(",")
                if data.get("bandit_include_tests")
                else [],
                "skip_tests": data.get("bandit_skip_tests", "").split(",")
                if data.get("bandit_skip_tests")
                else [],
        ...
        if "semgrep" in data.get("tools", []):
            security_config["semgrep"] = {
                ...
                "exclude_patterns": data.get("semgrep_exclude_patterns", "").split(",")
                if data.get("semgrep_exclude_patterns")
                else [],
                "include_patterns": data.get("semgrep_include_patterns", "").split(",")
                if data.get("semgrep_include_patterns")
                else [],
                "autofix": data.get("semgrep_autofix", False),
                "dry_run": data.get("semgrep_dry_run", False),
                "oss_only": data.get("semgrep_oss_only", True),
                "products": data.get("semgrep_products", "").split(",")
                if data.get("semgrep_products")
                else ["code"],
    ...
    # ZAP Scan Configuration
        ...
        zap_config = {
            ...
            "listen_port": int(data.get("zap_listen_port", 8080))
            if data.get("zap_listen_port")
            else None,
            "passive_scan_delay": int(data.get("zap_passive_scan_delay", 0))
            if data.get("zap_passive_scan_delay")
            else 0,
            ...
            "max_time": int(data.get("zap_max_time", 0))
            if data.get("zap_max_time")
            else None,
        ...
        if zap_config["scan_type"] == "api":
            ...
            zap_config["api_format"] = data.get(
                "zap_api_format", "openapi"
            )  # openapi, soap, graphql
    ...
    # Performance Testing Configuration
        ...
        performance_config = {
            ...
            "web_port": int(data.get("perf_web_port", 8089))
            if data.get("perf_web_port")
            else None,
            "headless": data.get("perf_headless", True),
            "csv_output": data.get("perf_csv_output", False),
            "html_output": data.get("perf_html_output", True),
            "tags": data.get("perf_tags", "").split(",")
            if data.get("perf_tags")
            else [],
            "exclude_tags": data.get("perf_exclude_tags", "").split(",")
            if data.get("perf_exclude_tags")
            else [],
            "stop_timeout": int(data.get("perf_stop_timeout", 0))
            if data.get("perf_stop_timeout")
            else None,
        }
    ...
    # AI Analysis Configuration
        ...
        ai_config = {
            ...
            "focus_areas": data.get("ai_focus_areas", "").split(",")
            if data.get("ai_focus_areas")
            else [],
            ...
            "max_tokens": int(data.get("ai_max_tokens", 4000))
            if data.get("ai_max_tokens")
            else None,
            "temperature": float(data.get("ai_temperature", 0.7))
            if data.get("ai_temperature")
            else None,
        }
...
def testing_api_test_details(test_id):
    ...
    try:
        ...
        if not test:
            if request.headers.get("HX-Request"):
                return render_template(
                    "partials/test_details.html", test=None, error="Test not found"
                )
            else:
        ...
        if hasattr(test, "total_tasks") and test.total_tasks and test.total_tasks > 0:
            ...
            test_dict["progress_percentage"] = round(
                (completed / test.total_tasks) * 100, 1
            )
        else:
        ...
        if request.headers.get("HX-Request"):
            return render_template(
                "partials/test_details.html", test=None, error=str(e)
            )
        else:
            return jsonify({"success": False, "error": str(e)}), 500
...
def testing_api_test_results(test_id):
    ...
    try:
        ...
        if not test:
            if request.headers.get("HX-Request"):
                return render_template(
                    "partials/test_results.html", test=None, error="Test not found"
                )
            else:
                return jsonify({"success": False, "error": "Test not found"}), 404
        ...
        if test.status and test.status.value == "completed":
            ...
            test_dict["results"] = {
                ...
                "safety": {
                    ...
                    "metrics": {"packages_checked": 45, "vulnerabilities_found": 1},
                },
            }
            ...
            test_dict["error_message"] = (
                "Connection timeout while scanning target application"
            )
        ...
        if request.headers.get("HX-Request"):
            return render_template(
                "partials/test_results.html", test=None, error=str(e)
            )
        else:
            return jsonify({"success": False, "error": str(e)}), 500
    ...
    try:
        ...
        if action == "restart":
            if not job.can_be_restarted():
                return jsonify(
                    {"success": False, "error": "Job cannot be restarted"}
                ), 400
            ...
            if job.auto_start:
                try:
                    ...
                    if service_result.get("success"):
                        ...
                        db.session.commit()
                        logger.info(
                            f"Job {test_id} restarted and submitted to unified CLI analyzer"
                        )
                    else:
                        logger.warning(
                            f"Failed to submit restarted job to service: {service_result.get('error')}"
                        )
                except Exception as e:
            ...
            result = {
                "success": True,
                "message": f"Job {job.name} restarted successfully",
            }
            ...
            tests_data.append(
                {
                    ...
                    "type_color": "primary"
                    if test.get_primary_job_type() == "security"
                    else "info",
...
def api_test_history():
    ...
    try:
        ...
        recent_tests = (
            ...
            .order_by(BatchJob.completed_at.desc())
            .limit(20)
            .all()
        )
        ...
        for service_name in service_names:
            try:
                ...
                if containers:
                    ...
                    status = (
                        "running"
                        if "running" in str(container_status).lower()
                        else "stopped"
                    )
                    port = None
...
def internal_error(error):
    ...
    if "strftime" in error_str or "template" in error_str.lower():
        if request.headers.get("HX-Request") == "true":
            return (
                f'<div class="alert alert-danger">Server Error: {error_str}</div>',
                500,
            )
        else:
        ...
        try:
            ...
            if model and model.model_name:
                ...
                if "claude" in display_name.lower():
                    ...
                    if "claude" in display_name and not display_name.startswith(
                        "Claude"
                    ):
...
def api_simple_models():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": models_data, "total_count": len(models_data)}
        )
...
def register_blueprints(app):
    ...
    app.register_blueprint(
        testing_bp
    )  # Add testing blueprint for template compatibility
