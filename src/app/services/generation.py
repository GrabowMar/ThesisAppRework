...
"""
    ...
    def _format_api_endpoints(
        ...
        def _normalize_path(path: str) -> str:
            if not path or generation_mode != 'guarded':
                return path
            if admin:
                if path.startswith('/api/admin'):
                    normalized = path[len('/api/admin'):] or '/'
        ...
        for endpoint in endpoints:
            ...
            raw_path = endpoint.get('path', '/api/example')
    ...
    def _build_prompt(self, config: GenerationConfig) -> str:
        ...
        if template and reqs:
            ...
            if query_type == 'admin':
                ...
                endpoint_text = self._format_api_endpoints(reqs, admin=True, generation_mode=generation_mode)
            else:
                if config.component == 'backend':
                    req_list = reqs.get('backend_requirements', [])
                else:
                    req_list = reqs.get('frontend_requirements', [])
                endpoint_text = self._format_api_endpoints(reqs, admin=False, generation_mode=generation_mode)
            
            # Also provide user endpoints for admin context
            admin_endpoint_text = (
                self._format_api_endpoints(reqs, admin=True, generation_mode=generation_mode)
                if query_type == 'admin'
                else ''
            )
    ...
    def _build_fallback_prompt(self, config: GenerationConfig, reqs: Optional[Dict]) -> str:
        ...
        endpoint_text = self._format_api_endpoints(reqs, generation_mode=config.generation_mode)
...
class GenerationService:
    ...
    def __init__(self):
        raise RuntimeError(
            "Legacy GenerationService has been removed. Use generation_v2 (get_generation_service) instead."
        )
