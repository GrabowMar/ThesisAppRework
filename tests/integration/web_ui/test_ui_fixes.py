...
def test_web_ui_analysis_with_fixed_paths():
    """Test web UI analysis creation and result paths."""
    print("=" * 70)
    ...
    assert response.status_code == 302, f"Failed to create analysis: {response.status_code}"
    ...
    results_dir = Path(__file__).parent.parent.parent.parent / 'results' / model_slug / f'app{app_number}'

    print(f"Checking results in: {results_dir}")
    print()

    assert results_dir.exists(), f"Results directory doesn't exist: {results_dir}"

    # List all task directories
    task_dirs = sorted(results_dir.glob('task_*'), key=lambda p: p.stat().st_mtime, reverse=True)

    assert len(task_dirs) > 0, f"No task directories found in {results_dir}"
    ...
    if task_dirs:
        ...
        if 'task_task_' in latest.name:
            ...
            assert False, "Double 'task_' prefix bug detected"
        else:
