...
async def verify_sarif_extraction():
    ...
    # Create test task
    test_task_id = "sarif_extraction_test"
    ...
    print(f"📊 New size: {new_size:.2f} KB ({new_filepath.stat().st_size / 1024 / 1024:.2f} MB)")
    ...
    print(f"📊 Total SARIF size: {total_sarif_size:.2f} KB")
    ...
    print(f"   SARIF references: {sarif_refs}")
    
    if sarif_refs != len(sarif_files):
        print(f"⚠️  Warning: Reference count ({sarif_refs}) doesn't match file count ({len(sarif_files)})")
    
    print(f"\n{'✅' if reduction_percent > 90 else '⚠️ '} Optimization {'successful' if reduction_percent > 90 else 'partial'}: {reduction_percent:.1f}% reduction")
